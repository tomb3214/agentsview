	...
	tests := []struct {
		...
		},
		{
			...
			wantParts: []string{
				...
				"machine IN ($3,$4)",
				"agent IN ($5,$6)",
				"COALESCE(ended_at, started_at, created_at) >= $7::timestamptz",
				"COALESCE(LEAST(started_at, created_at), started_at, created_at) < $8::timestamptz",
				"COALESCE(ended_at, started_at, created_at) >= $9::timestamptz",
				"COALESCE(LEAST(started_at, created_at), started_at, created_at) < $10::timestamptz",
