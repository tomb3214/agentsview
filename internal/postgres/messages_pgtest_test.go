...
func TestPGSearchDeferredUnicodeSnippetsAndPage(t *testing.T) {
	...
	assert.Equal(t, "snippet-name", first.Results[0].SessionID)
	assert.Equal(t, -1, first.Results[0].Ordinal)
	assert.Equal(t, query+" title", first.Results[0].Snippet)
	require.Equal(t, 1, first.NextCursor)
	filter.Cursor = first.NextCursor
	second, err := store.Search(context.Background(), filter)
	require.NoError(t, err)
	require.Len(t, second.Results, 1)
	assert.Equal(t, "snippet-early", second.Results[0].SessionID)
	assert.Equal(t, 3, second.Results[0].Ordinal)
	assert.Equal(t, string([]rune(early)[:200])+"...", second.Results[0].Snippet)
	...
	assert.Equal(t, "snippet-late", last.Results[0].SessionID)
	assert.Equal(t, "..."+string([]rune(late)[70:270])+"...", last.Results[0].Snippet)
