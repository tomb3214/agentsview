...
func TestPushPreservesUnchangedToolResults(t *testing.T) {
  ...
  WHERE p.session_id=$1 AND m.ordinal=0 AND p.note='retained pin'`, id).Scan(&pinCount))
	...
	push(false)
	changed := event()
	assert.Equal(t, original.ID, changed.ID, "changed event keeps its logical key")
	assert.NotEqual(t, original.CTID, changed.CTID, "changed event/context must update stale data")
	parity()
	push(true)
